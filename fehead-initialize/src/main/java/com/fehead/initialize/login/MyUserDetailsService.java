...
public class MyUserDetailsService implements UserDetailsService, TelUserDetailService {
    ...
    public UserDetails loadUserByUsername (String username) throws UsernameNotFoundException {
        ...
        if (CheckEmailAndTelphoneUtil.checkEmail(username)) {
            ...
            UserDO userDO = userDOMapper.selectByThirdPartyId(username);
            username = userDO.getId().toString();
            password = passwordEncoder.encode(userDO.getAvatar());
        } else {
            throw new UsernameNotFoundException("手机号或邮箱或易班ID格式错误");
        }
