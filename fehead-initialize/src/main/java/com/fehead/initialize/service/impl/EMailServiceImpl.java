...
public class EMailServiceImpl implements EMailService {
    ...
    public void sendValidateEmail(String toAddress) throws MessagingException, BusinessException {
        ...
        if(redisService.exists(key)){
            if(!emailCode.isExpired(emailTime)){//是否能重复发送
                logger.info("邮件已发送");
    ...
    @Override
    public boolean validateEmailCode(String yourEmail, String code) {

        String codeInRedis = "";

        if (!redisService.exists(securityProperties.getSmsProperties().getLoginPreKeyInRedis() + yourEmail)) {
            throw new UsernameNotFoundException("验证码不存在");
        } else {
            codeInRedis =((ValidateCode)redisService.get(securityProperties.getSmsProperties().getLoginPreKeyInRedis() + yourEmail)).getCode();
        }
